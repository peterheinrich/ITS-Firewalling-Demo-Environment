...
in-target /bin/sh -c -- "apt install -y -q vim mc net-tools tcpdump apache2"
...
in-target /bin/sh -c -- 'echo "    address 172.16.0.2" >> /etc/network/interfaces'
...
in-target /bin/sh -c -- 'echo "    gateway 172.16.0.1" >> /etc/network/interfaces'
